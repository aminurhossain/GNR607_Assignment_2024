...
class ImageApp:
    def __init__(self, root):
        ...
        self.regenerate_button.grid(row=0, column=5, padx=5)

        self.showdiff_button = tk.Button(self.button_frame, text="Show Diff", command=self.compute_and_display_pca2)
        self.showdiff_button.grid(row=0, column=8, padx=5)
        ...
        self.help_button.grid(row=0, column=9, padx=5)
    ...
    #     if self.input_image_org is None or self.input_image_synthesis is None:
    #         messagebox.showwarning("Warning", "Load an image and perform PCA reconstruction first.")
    #         return
    #
    #     try:
    #         # Calculate the absolute difference
    #         input_array = self.input_image_org.astype(np.float32)
    #         reconstructed_array = self.input_image_synthesis.astype(np.float32)
    #
    #         difference = np.abs(input_array - reconstructed_array)
    #
    #         # Normalize the difference for visualization
    #         normalized_diff = difference / np.max(difference) * 255.0
    #         normalized_diff = normalized_diff.astype(np.uint8)
    #
    #         # Convert to PIL Image format for display
    #         diff_image = Image.fromarray(normalized_diff[:, :, [4, 3, 2]])
    #
    #         # Clear the output canvas
    #         self.canvas_output.delete("all")
    #
    #         # Resize and display the difference image
    #         width_output, height_output = int(diff_image.width * self.zoom_factor), int(
    #             diff_image.height * self.zoom_factor)
    #         resized_diff_image = diff_image.resize((width_output, height_output), Image.Resampling.LANCZOS)
    #         self.output_image_tk = ImageTk.PhotoImage(resized_diff_image)
    #
    #         self.canvas_output_image = self.canvas_output.create_image(0, 0, anchor=tk.NW, image=self.output_image_tk)
    #         self.canvas_output.config(scrollregion=self.canvas_output.bbox(self.canvas_output_image))
    #
    #         # Update caption for the output canvas
    #         self.output_caption.config(text="Difference Image")
    #     except Exception as e:
    #         messagebox.showerror("Error", f"Failed to compute difference image: {str(e)}")
    ...
    def compute_and_display_pca2(self):
        if self.input_image is None:
            return
        ...
        reconstructed_image = self.input_image_synthesis
        # Reshape the image back to its original HxWxC shape
        # img_array_part = img_array[:,:,[4,3,2]]
        # Convert the PCA image back to PIL format
        # img_array = np.transpose(img_array, (1, 2, 0))
        img_array_part = img_array[:, :, [4, 3, 2]]
        reconstructed_image_part = reconstructed_image[:,:,[4,3,2]]
        diff_img = np.uint8(np.abs(img_array_part - reconstructed_image_part))
        # diff_img = convert_16bit_to_8bit_minmax()
        self.pca_image = Image.fromarray(diff_img)
